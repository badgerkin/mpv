...
static bool integer_mode_s24_hack(struct ao *ao)
{
...
static int find_best_format(struct ao *ao, AudioStreamBasicDescription *out_fmt)
{
    ...
    *out_fmt = (AudioStreamBasicDescription){0};
    ...
    int temp;
    CA_GET_O(p->device, kAudioDevicePropertyTransportType, &temp);

    if (temp == (kAudioDeviceTransportTypeBluetooth | kAudioDeviceTransportTypeBluetoothLE)){
        if (!out_fmt->mFormatID || ca_bluetooth_asbd_is_better(&asbd, out_fmt, stream_asbd))
        *out_fmt = *stream_asbd;
    }else if (temp == kAudioDeviceTransportTypeBuiltIn & asbd.mBitsPerChannel == 16){ 
    ...
    if (p->integer_mode) {
        //if (!integer_mode_s24_hack(ao) && (asbd.mBitsPerChannel == 24)) {
        ...
        *out_fmt = *stream_asbd;}
        ...
        //if (!integer_mode_s24_hack(ao) && (asbd.mBitsPerChannel == 24) && (p->integer_mode)) 
        //MP_INFO(ao, "MSB padding in Integer Mode is not implemented.\n");
    talloc_free(formats);
...
static int init(struct ao *ao)
{
    ...
    }else if (p->buffersize == 1024 ){  //Appearently this is not the correct way to do it.
    ...
    CHECK_CA_WARN("failed to set Saving Power Mode");
    ...
    AudioStreamBasicDescription asbd;
    ca_fill_asbd(ao, &asbd);
    if (!integer_mode_s24_hack(ao) && (p->stream_asbd.mFormatFlags & kAudioFormatFlagIsNonMixable) && (asbd.mBitsPerChannel == 32)) {
    new_format = ca_asbd_to_mp_format_integer_mode_hack(&p->stream_asbd);
...
const struct ao_driver audio_out_coreaudio_exclusive = {
    ...
    .priv_defaults = &(const struct priv){
        ...
        .buffersize = 1024, // Default value is 512, increase to 1024 as per Apple's Suggestion (https://developer.apple.com/library/archive/technotes/tn2321/_index.html)
        .IOCycleUsage = 1,
        .integer_mode = 1,
    },
    .options = (const struct m_option[]){
        OPT_FLAG("spdif-hack", spdif_hack, 0),
        OPT_INTRANGE("buffer-size", buffersize, 0, 1, 24576), // Not sure how to set min and max values which are retrieved from "OSStatus err2 = CA_GET_O(device, kAudioDevicePropertyBufferFrameSizeRange, &value_range);"
